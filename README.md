...
3. **macOS**: the app is not notarized, so macOS Gatekeeper will block the first launch. Right-click (or Control-click) the app and select **Open**, then click **Open** again in the dialog. Alternatively, run `xattr -cr /Applications/Handy.app` in Terminal. You only need to do this once.
